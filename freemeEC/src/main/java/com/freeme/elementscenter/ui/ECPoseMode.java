...
public class ECPoseMode extends ECBackHandledFragment implements View.OnClickListener,
    ...
    private synchronized void handleOnlineData(int index, List<ECItemData> list) {
        ...
        if (index == ECUtil.POSEMODE_TYPE_ARRAY.length - 1 && list != null) {
            showViewByStatus(1);
        }
        if (list != null) {
            ...
            String jsonStr = ECUtil.ecItemDataToJsonStr(ECUtil.POSEMODE_TYPE_ARRAY[index], list);
            if (!TextUtils.isEmpty(jsonStr)) {
                ECUtil.saveJsonStrToFile(ECUtil.POSEMODE_TYPE_ARRAY[index], jsonStr);
            }
    ...
    private void requestDataByType(int type) {
        ...
        ECOnlineData dataTask = new ECOnlineData(ECUtil.POSE_TYPE_CODE, type);
    ...
    public void onDataReady(int typeCode, int pageItemTypeCode, List<ECItemData> dataList) {
        ...
        if (typeCode == ECUtil.POSE_TYPE_CODE) {
