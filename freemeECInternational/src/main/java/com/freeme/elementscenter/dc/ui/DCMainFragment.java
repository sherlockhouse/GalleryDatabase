...
public class DCMainFragment extends ECBackHandledFragment implements OnClickListener,
    ...
    private void requestAdverOnlineData() {
        ...
        try {
            ...
            paraInfo.put("channel", "sphwdroi");
            paraInfo.put("language", ECUtil.getCurrLocaleLanguage());
            ...
            protected void onPostExecute(List<AdvertiseItem> result) {
                ...
                if (result != null && result.size() > 0) {
                    ...
                    if (!TextUtils.isEmpty(jsonStr)) {
                        String fileName = ECUtil.getCurrLocaleLanguage() + "-" + "advertise";
                        ECUtil.saveJsonStrToFile(fileName, jsonStr);
                    }
                } else {
                    mIsBannerRequestSuccess = false;
                }
    ...
    private void requestData() {
        if (ECFragmentUtil.NetWorkStatus(getActivity())) {
            ...
            String language = ECUtil.getCurrLocaleLanguage() + "-";
            String jsonStr = ECUtil.readJsonStrFromFile(language + "advertise");
            if (!TextUtils.isEmpty(jsonStr)) {
            ...
            jsonStr = ECUtil.readJsonStrFromFile(language + "plugin" + ECMainActivity.sCameraId);
            if (!TextUtils.isEmpty(jsonStr)) {
