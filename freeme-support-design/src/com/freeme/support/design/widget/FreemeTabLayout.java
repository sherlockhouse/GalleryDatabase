...
public class FreemeTabLayout extends HorizontalScrollView {
    ...
    public FreemeTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mTabTextSize = ta.getResourceId(0,0);
            mTabTextColors = getResources().getColorStateList(ta.getResourceId(1,0));
        } finally {
            ta.recycle();
        }
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        protected void onLayout(boolean changed, int l, int t, int r, int b) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (child instanceof TabView) {
                    TextView text = ((TabView) child).getTextWidget();
                    if (text != null) {
                        mTabContentHeight = text.getHeight();
                        break;
                    }
                }
            }
        ...
        public void draw(Canvas canvas) {
            ...
            // Thick colored solid circle below the current selection
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                canvas.drawCircle(
                        (mIndicatorLeft + mIndicatorRight) / 2f,
                        (getHeight() * 3 + mTabContentHeight) / 4f,
                        mSelectedIndicatorHeight / 2f,
