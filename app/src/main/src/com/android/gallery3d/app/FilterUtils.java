...
public class FilterUtils {
    ...
    //*/ Added by xueweili for add filter key, 2015-7-23
    public static final  int FILTER_CAN_VISIBLE   = 6;
    public static final  int FILTER_HIDE_CAMERA   = 7;
    ...
    //*/ Added by xueweili for add filter function, 2015-7-23
    public static String switchFilterPath(String base, int filterType) {
        return newFilterPathVisible(removeOneFilterFromPath(base), filterType);
    }

    public static String removeOneFilterFromPath(String base) {
        boolean[] done = new boolean[1];
    ...
    private static String removeOneFilterFromPath(String base, boolean[] done) {
        if (done[0]) return base;
        String[] segments = Path.split(base);
        if (segments[0].equals("filter")) {
            done[0] = true;
            return Path.splitSequence(segments[2])[0];
        }
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < segments.length; i++) {
            sb.append("/");
    ...
    public static String newFilterPathVisible(String base, int filterType) {
        String kind = "";
        switch (filterType) {
            case FILTER_HIDE_CAMERA:
                kind = "visible_without_camera";
                break;
            case FILTER_CAN_VISIBLE:
                kind = "visible";
                break;
            default:
                break;
