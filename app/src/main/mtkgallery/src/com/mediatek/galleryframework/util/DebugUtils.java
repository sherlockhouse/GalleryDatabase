...
//    SystemProperties.getInt("Gallery_DEBUG_HQS", 0) == 1 ? true : false;
...
//            "Gallery_DEBUG_RENDER", 0) == 1 ? true : false;
    ...
    public static void dumpBitmap(Bitmap bitmap, String fileName) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.i(TAG, "<dumpBitmap> IOException", e.getCause());
        } finally {
            try {
                if (fos != null) {
                    fos.close();
                ...
                e.printStackTrace();
                Log.i(TAG, "<dumpBitmap> close FileOutputStream", e.getCause());
            }
        }
    }
}
