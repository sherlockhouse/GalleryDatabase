...
public class PermissionHelper {
    ...
    public static boolean checkAndRequestForGallery(Activity activity) {
        ...
        if (permissionsNeedRequest.size() == 0) {
            com.freeme.gallery.util.Log.i(TAG, "<checkAndRequestForGallery> all permissions are granted");
            return true;
        } else {
            com.freeme.gallery.util.Log.i(TAG, "<checkAndRequestForGallery> not all permissions are granted, reuqest");
    ...
    public static boolean checkAndRequestForLocationCluster(Activity activity) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.ACCESS_FINE_LOCATION)
                != PackageManager.PERMISSION_GRANTED) {
            com.freeme.gallery.util.Log.i(TAG, "<checkAndRequestForLocationCluster> permission not granted, reuqest");
            ActivityCompat.requestPermissions(activity,
                    new String[]{Manifest.permission.ACCESS_FINE_LOCATION}, 0);
            return false;
        }
        com.freeme.gallery.util.Log.i(TAG, "<checkAndRequestForLocationCluster> all permissions are granted");
        return true;
    }
    ...
    public static boolean checkAndRequestForWidget(Activity activity) {
        if (!checkStoragePermission(activity)) {
            com.freeme.gallery.util.Log.i(TAG, "<checkAndRequestForWidget> permission not granted, reuqest");
        ...
        com.freeme.gallery.util.Log.i(TAG, "<checkAndRequestForWidget> all permissions are granted");
        return true;
    }
    ...
    public static boolean checkForFilterShow(Activity activity) {
        if (!checkStoragePermission(activity)) {
            com.freeme.gallery.util.Log.i(TAG, "<checkForFilterShow> permission not granted, finish");
            PermissionHelper.showDeniedPrompt(activity);
            activity.finish();
            return false;
        }
        com.freeme.gallery.util.Log.i(TAG, "<checkForFilterShow> all permissions are granted");
        return true;
    }
    ...
    public static boolean checkStoragePermission(Context context) {
        ...
        com.freeme.gallery.util.Log.i(TAG, "<checkStoragePermission> writeGranted = " + writeGranted
