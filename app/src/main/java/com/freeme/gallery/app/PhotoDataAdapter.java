...
import com.freeme.gallery.ui.ScreenNail;
import com.freeme.gallery.ui.SynchronizedHandler;
import com.freeme.gallery.ui.TileImageViewAdapter;
...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private int  mFocusHintDirection = PhotoView.Model.FOCUS_HINT_NEXT;
    ...
    public int getLoadingState(int offset) {
        ...
        if (entry == null) return PhotoView.Model.LOADING_INIT;
        if (entry.failToLoad) return PhotoView.Model.LOADING_FAIL;
        if (entry.screenNail != null) return PhotoView.Model.LOADING_COMPLETE;
        return PhotoView.Model.LOADING_INIT;
    }
            ...
            while (mActive) {
                ...
                if (index == MediaSet.INDEX_NOT_FOUND) {
                    ...
                    if (index == (mCameraIndex + 1)) {
                        focusHintDirection = PhotoView.Model.FOCUS_HINT_NEXT;
                    }
                    if (focusHintDirection == PhotoView.Model.FOCUS_HINT_PREVIOUS
                            && index > 0) {
                        index--;
                    }
                }
