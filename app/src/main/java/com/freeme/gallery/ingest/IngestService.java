...
package com.freeme.gallery.ingest;
...
import com.freeme.gallery.ingest.data.ImportTask;
import com.freeme.gallery.ingest.data.IngestObjectInfo;
import com.freeme.gallery.ingest.data.MtpClient;
import com.freeme.gallery.ingest.data.MtpDeviceIndex;
...
public class IngestService extends Service implements ImportTask.Listener,
        ...
        if (mClientActivity == null) {
            if (mNeedRelaunchNotification) {
                mNotificationBuilder.setProgress(0, 0, false)
                        .setContentText(getResources().getText(com.freeme.gallery.R.string.ingest_scanning_done));
                mNotificationManager.notify(com.freeme.gallery.R.id.ingest_notification_scanning,
                        mNotificationBuilder.build());
            }
            return;
        }
        mNotificationManager.cancel(com.freeme.gallery.R.id.ingest_notification_importing);
        mNotificationManager.cancel(com.freeme.gallery.R.id.ingest_notification_scanning);
    ...
    protected void importSelectedItems(SparseBooleanArray selected, Adapter adapter) {
        ...
        mNotificationBuilder.setProgress(0, 0, true)
                .setContentText(getResources().getText(com.freeme.gallery.R.string.ingest_importing));
        startForeground(com.freeme.gallery.R.id.ingest_notification_importing,
                mNotificationBuilder.build());
    ...
    public void onImportProgress(int visitedCount, int totalCount,
        ...
        mNotificationBuilder.setProgress(totalCount, visitedCount, false)
                .setContentText(getResources().getText(com.freeme.gallery.R.string.ingest_importing));
        mNotificationManager.notify(com.freeme.gallery.R.id.ingest_notification_importing,
                mNotificationBuilder.build());
    }
    ...
    public void onImportFinish(Collection<IngestObjectInfo> objectsNotImported,
        ...
        if (mClientActivity != null) {
            ...
            mNotificationBuilder.setProgress(0, 0, false)
                    .setContentText(getResources().getText(com.freeme.gallery.R.string.ingest_import_complete));
            mNotificationManager.notify(com.freeme.gallery.R.id.ingest_notification_importing,
                    mNotificationBuilder.build());
        }
    }
        ...
        if (mClientActivity != null) {
            ...
            if (currentTime > mLastProgressIndexTime + PROGRESS_UPDATE_INTERVAL_MS) {
                ...
                mNotificationBuilder.setProgress(0, numVisited, true)
                        .setContentText(getResources().getText(com.freeme.gallery.R.string.ingest_scanning));
                mNotificationManager.notify(com.freeme.gallery.R.id.ingest_notification_scanning,
                        mNotificationBuilder.build());
            }
        }
