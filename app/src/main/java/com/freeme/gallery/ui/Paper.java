...
public class Paper {
    ...
    //*/ Added by droi Linguanrong for Gallery new style, 2013-12-30
    public float[] getTransformH(Rect rect, float scrollY) {
        float top = mAnimationLeft.getValue();
        float bottom = mAnimationRight.getValue();
        float screenY = rect.centerY() - scrollY;
        float y = screenY + mHeight / 4;
        int range = 3 * mHeight / 2;
        float t = ((range - y) * top - y * bottom) / range;
        float degrees = (1 / (1 + (float) Math.exp(-t * ROTATE_FACTOR)) - 0.5f) * 2 * 30;
