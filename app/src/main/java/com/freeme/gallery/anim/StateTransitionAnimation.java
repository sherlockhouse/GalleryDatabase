...
package com.freeme.gallery.anim;
...
public class StateTransitionAnimation extends Animation {
    ...
    private static final int DURATION_IN  = 350;
    ...
    public StateTransitionAnimation(Transition t, RawTexture oldScreen) {
        ...
        //*/ Added by droi Linguanrong for gallery pick animation, 2014-9-3
        if (Transition.Outgoing == t) {
            mTransitionSpec.duration = DURATION_OUT;
        } else {
            mTransitionSpec.duration = DURATION_IN;
        }
        //*/
    }
    ...
    private void applyOldTexture(GLView view, GLCanvas canvas, float alpha, float scale, boolean clear) {
        ...
        //*/ Modified by droi Linguanrong for gallery pick animation, 2014-9-3
        int xOffset = mOffsetX;//view.getWidth() / 2;
        int yOffset = mOffsetY;//view.getHeight() / 2;
        //*/
    ...
    public void applyContentTransform(GLView view, GLCanvas canvas) {
        //*/ Modified by droi Linguanrong for change activity state animate, 2014-6-6
        int xOffset = mOffsetX;//view.getWidth() / 2;
        int yOffset = mOffsetY;//view.getHeight() / 2;
        //*/
        canvas.translate(xOffset, yOffset);
