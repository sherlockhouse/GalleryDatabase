...
public class AsyncImageCache {
    ...
    //*/Added by tyd yuanchengye 20141129 for more than 128 threads
    ...
    private              ArrayList<TaskValue>      mTaskCache     = new ArrayList<TaskValue>();
    ...
    public void displayImage(ImageView imageView, Drawable loadingImage, int width,
        ...
        if (!mTaskCache.isEmpty()) {
            synchronized (mTaskCache) {
                for (TaskValue taskValue : mTaskCache) {
                    if (taskValue.asyncImageTask != null
                            && taskValue.asyncImageTask.mImageRef != null
                            && tag.equals(taskValue.asyncImageTask.mImageRef.tag)) {
                        //if (callback != null) 
                        //    callback.run();
                        return;
                    }
        ...
        //*/Modified by tyd yuanchengye 20141129, for more than 128 threads
        if (mTaskStack.size() > MAX_NUM_THREAD) {
            mTaskCache.add(new TaskValue(asyncImageTask, imageRef));
        } else {
    ...
    public static class TaskValue {
        ...
        public TaskValue(AsyncImageLoadTask asyncImageTask, ImageRef imageRef) {
            super();
    ...
    class AsyncImageLoadTask extends AsyncTask<ImageRef, Integer, Bitmap> {
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ...
            //*/Added by tyd yuanchengye 20141129, for more than 128 threads
            if (!mTaskCache.isEmpty()) {
                TaskValue taskValue = mTaskCache.get(0);
                mTaskStack.push(taskValue.asyncImageTask);
                taskValue.asyncImageTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, taskValue.imageRef);
                mTaskCache.remove(taskValue);
            }
            //*/
