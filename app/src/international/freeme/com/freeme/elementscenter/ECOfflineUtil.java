...
public final class ECOfflineUtil {
    ...
    private static boolean utf8CodeCheck(String text) {
        String sign = "";
        if (text.startsWith("%e")) for (int p = 0; p != -1;) {
            p = text.indexOf("%", p);
            if (p != -1) p++;
            sign += p;
        }
    ...
    public static List<ECOfflineItemData> getOfflineItemDataByType(String currLanguage,
            int typeCode, int pageType) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                String pageTypeName = split[2];
                if (pageTypeName.equals(pageTypeCode) && language.equals(currLanguage)) {
